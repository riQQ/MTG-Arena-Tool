class PointerFix {
  ...
  constructor({
    ...
    if (initialState === undefined) {
      ...
      if (top.readyState === "complete") {
        console.warn(
          "The pointer event fix should be activated on page load when initialState is not defined. Defaulting to ",
          (this.ignore = false),
          "."
        );
      } else {
  ...
  set active(active) {
    if (active && !this.#active) {
      ...
      this.root
        .getRootNode()
        .removeEventListener("pointerover", this.eventHandler);
    }
    this.#active = active;
  }
}
